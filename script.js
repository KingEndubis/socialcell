...
function previousWeek() {
  ...
  saveWithDialog(defaultName, json, 'JSON Plan');
}
...
function gatherInputs() {
  ...
  const currentFollowers = parseInt($("currentFollowers")?.value || '0', 10) || 0;
...
(function(){
  ...
  // Handle the button click to show the prompt
  if (installBtn) {
    installBtn.addEventListener('click', async () => {
      ...
      // Hide the button if installed or dismissed; we can re-show later on next event
  ...
  // Optionally hide the button once installed
